...
`NTIS_USE_MAP_REDUCE_JUDGING=true`(기본값)이면 Judge는 Map(경량 직렬화 + 경량 프롬프트, `max_tokens=400`, `survivors` 배열 반환) → Reduce(전체 직렬화 + 상세 프롬프트, `max_tokens` 제한 없음) 2단계로 동작한다. 이 구조로 입력 토큰을 절약하고 심사 대상을 축소한 뒤 집중 비교가 이루어진다.
