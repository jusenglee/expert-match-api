...
flowchart TD
    ...
    F --> G["후보 카드 생성"]
    G --> H1["Map: 배치별 경량 LLM 병렬 예비 심사"]
    H1 --> H2["Reduce: 생존자 모아 상세 LLM 최종 심사"]
    H2 --> I["추천 이유, 근거, 주의사항과 함께 응답"]
```
...
### 1-6. LLM이 최종 추천을 만드는 단계 (Map-Reduce 2단계 심사)
...
현재 LLM Judge는 후보 수가 많을 때 **Map-Reduce 방식으로 2단계에 걸쳐 심사**한다.

#### Map 라운드 (예비 심사)

숏리스트 전체(기본 40명)를 `NTIS_LLM_JUDGE_BATCH_SIZE`(기본 10명) 단위로 분할한 뒤, 각 배치를 **경량 프롬프트**로 병렬 호출한다.

Map 라운드의 특징은 아래와 같다.

- **경량 직렬화**: 후보의 프로필 핵심 정보(이름, 소속, 전공, 학위, 실적 건수, 논문/특허/과제 제목)만 전달한다. 초록, 키워드, 연구 요약 등은 제외하여 입력 토큰을 절약한다.
- **경량 프롬프트**: "관련성 높은 후보만 골라서 `survivors` 배열로 반환하라"는 단순한 지시만 전달한다. 추천 이유, 근거, 리스크 등 상세 설명은 요구하지 않는다.
- **max_tokens 제한**: 응답 토큰을 400으로 제한하여 불필요한 출력을 방지한다.
- **병렬 실행**: `asyncio.gather`로 모든 배치를 동시에 호출하되, `NTIS_LLM_JUDGE_MAX_CONCURRENCY`(기본 10) 세마포어로 동시 호출 수를 제한한다.
...
Map 라운드에서 살아남은 후보들만 모아 **상세 프롬프트**로 최종 1회 호출한다.
...
이 2단계 구조 덕분에 전체 토큰 비용이 크게 줄어들고, 심사 대상이 좁아진 상태에서 상세 비교가 이루어지므로 추천 품질도 향상된다.
...
### 1-7. LLM 응답에서 JSON을 안전하게 추출하는 방법

Planner와 Judge 모두 LLM에게 JSON 객체를 요구하지만, 실제 LLM 응답에는 마크다운 코드 펜스, `<thinking>` 태그, 줄바꿈 등 예상 밖의 텍스트가 섞일 수 있다.

이 서비스는 3단계 방어적 JSON 추출(`_extract_json_object_text`)을 Planner와 Judge 양쪽에서 동일하게 적용한다.

1. **`<thinking>` 블록 제거**: LLM이 `<thinking>...</thinking>`으로 내부 추론을 출력한 경우 정규식으로 완전 제거한다.
2. **마크다운 코드 블록 제거**: ` ```json ... ``` ` 형태의 코드 펜스가 있으면 내부 텍스트만 추출한다.
3. **균형 잡힌 중괄호 매칭**: 첫 번째 `{`부터 깊이(depth)를 추적하며, 문자열 리터럴 안의 `{}`는 무시하고, 깊이가 0으로 돌아오는 `}`에서 정확히 끊는다. 이 방식은 JSON 뒤에 추가 텍스트가 붙어 있어도 "Extra data" 파싱 오류 없이 올바른 JSON 객체만 추출한다.

이 3단계 방어 덕분에 LLM이 순수 JSON만 반환하지 않더라도 파싱 실패 → 휴리스틱 폴백으로 빠지는 비율이 크게 줄어든다.

### 1-8. Planner 프롬프트 설계 원칙

Planner 프롬프트는 다음 원칙으로 설계되어 있다.

- **JSON-only 출력 강제**: "첫 글자는 반드시 `{`이고 마지막 글자는 반드시 `}`" 규칙을 명시한다. 설명, 마크다운, 코드 펜스 출력을 금지한다.
- **스키마 명시**: `PlannerOutput`의 전체 필드(`intent_summary`, `core_keywords`, `hard_filters`, `exclude_orgs`, `soft_preferences`, `branch_weights`, `branch_query_hints`, `top_k`)를 예시와 함께 제시한다.
- **hard_filters 허용 키 제한**: `degree_slct_nm`, `art_recent_years`, `patent_cnt_min` 등 허용된 키만 사용 가능하도록 명시하고, 전공·연구 분야 단어는 `core_keywords`나 `branch_query_hints`에 넣도록 안내한다.
- **Few-shot 예시 2건**: 실제 질의와 정답 JSON을 2건 포함하여 LLM이 출력 형식을 정확히 따르도록 유도한다.
- **`<thinking>` 태그 미사용**: CoT(Chain-of-Thought) 사고 과정을 별도 태그로 유도하지 않는다. 이는 LLM이 `<thinking>` 블록 안에 `{}`를 생성하여 JSON 추출을 방해하는 문제를 원천 차단하기 위한 설계이다.
...
graph TD
    ...
    SVC --> P[질의 의도 분석기]
    P --> P1[LLM - JSON-only 강제 출력 + Few-shot 예시 또는 휴리스틱 기반 의도 파악]
    P1 --> P2[의도, 핵심 키워드, 엄격한 제약조건 필터, 브랜치별 검색 보정 힌트 도출]
    ...
    J_MAP --> J_RED[Reduce: 생존자만 모아 상세 근거 포함 전체 직렬화 후 2차 최종 판정]
...
graph LR
    ...
    subgraph S3[LLM 심사 경로 - Map-Reduce 병렬 처리]
        D --> I[전체 후보 40명을 N명 단위 청크로 분할]
        I --> I1[경량 직렬화: 프로필+제목만, 초록/키워드 제외]
        I1 --> J_MAP[각 청크별 경량 프롬프트로 LLM 비동기 병렬 호출 - Map<br>max_tokens=400, survivors 배열만 반환]
        J_MAP --> J_MERGE[생존자 expert_id 기준 전체 직렬화 재구성]
        J_MERGE --> J_RED[상세 프롬프트로 LLM 최종 심사 - Reduce<br>추천순위/이유/근거/리스크 모두 생성]
        J_RED --> K[3단계 JSON 추출 + 정규화 후 최종 선발 항목 반환]
    end
...
| 한글 블록 명칭 | 소스코드 클래스 | 역할 |
|---|---|---|
| **추천 파이프라인 총괄** | `RecommendationService` | Planner → Retriever → Judge 총지휘. 단계별 소요시간 추적. |
| **질의 의도 분석기** | `Planner` | `hard_filters`, `core_keywords`, `branch_query_hints` JSON 추출. JSON-only 강제 + Few-shot 예시 2건. |
| **검색 조건 변환기** | `QdrantFilterCompiler` | Planner 출력을 Qdrant Filter 구문으로 변환. |
| **특화 검색 쿼리 생성기** | `QueryTextBuilder` | 브랜치별 최적화된 검색어 생성. |
| **하이브리드 벡터 검색 수행기** | `QdrantHybridRetriever` | Dense + Sparse BM25 동시 검색, RRF 결합. |
| **전문가 프로필 카드 조립기** | `CandidateCardBuilder` | 검색 결과를 LLM 입력용 카드 객체로 압축. |
| **지능형 판정 / 고속 판정** | `Judge` | LLM 모드: Map(경량, max_tokens=400) → Reduce(상세) 2단계. Rule 모드: 점수 기반 고속 판정. 양쪽 모두 3단계 JSON 추출 적용. |
