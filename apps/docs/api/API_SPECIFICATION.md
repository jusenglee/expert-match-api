...
- **`query`** (string, 필수): 자연어 검색 질의
  - 여러 줄 텍스트가 들어오면 서버는 각 줄을 정리한 뒤 `, `로 이어 붙여 단일 질의로 정규화합니다.
  ...
  "candidates": [
    {
      ...
      "name": "홍길동",
      "organization": "A대학교",
      "branch_presence_flags": {"art": true, "pat": false, "pjt": true},
...
- **`candidates`** (list[object]): 심사 이전 단계에서 랭킹이 매겨진 전문가 초기 후보군(숏리스트) 객체 배열
  ...
  - `branch_presence_flags` (dict): 논문("art"), 특허("pat"), 과제("pjt") 등 어떤 영역의 실적 데이터를 보유하고 있는지에 대한 불리언 플래그맵
