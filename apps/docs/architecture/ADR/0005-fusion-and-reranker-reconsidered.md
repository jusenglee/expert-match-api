...
승인 (2026-05-28). chunk 재설계 시 "일부 고정 제약 재검토 개방"(사용자 결정)에 따른 명문화.
...
1. **Qdrant 융합 = equal RRF 유지.** 컬렉션 내 dense+sparse 융합은 `FusionQuery(RRF)` 고정. Qdrant 단 weighted RRF/score 가중합은 쓰지 않는다(스케일 차이에 강건, 재현성).
2. **집계 가중(doc_type prior) = 옵트인, 기본 equal.** 연구자 집계 단계의 family/doc_type 가중은 앱단 랭크 누적 가중으로 조정 가능하되 기본 equal(`NTIS_DOC_TYPE_PRIORS` 미설정). v1.x도 앱단 `BRANCH_WEIGHTS`를 이미 썼으므로 일관적이다.
3. **후보 cross-encoder 리랭커 = 기본 OFF, 옵트인 실험만.** 후보 순위의 척추는 RRF로 유지. `NTIS_CANDIDATE_RERANKER=band`로 켜더라도 **score 동률 밴드 내 재배열만** 허용하고 탈락·생성은 금지.
