...
# 평가위원 추천 RAG 시스템
## 설계 · 구현 · 제한 · 방향성 지침서 (chunk 재설계)

**기준선:** Qdrant 하이브리드 검색 / chunk 단위 Point / 단일 dense+sparse 벡터 / LLM 추천
**문서 버전:** v2.0 (chunk 재설계, 2026-05-28)
**선행 문서:** [`DATA_MODEL.md`](DATA_MODEL.md) — 본 문서의 데이터 전제는 모두 여기서 확정된다.

---

## 0. v2.0가 v1.x에서 바꾼 것 / 유지한 것

| 구분 | v1.x | v2.0 |
|---|---|---|
| **저장 단위** | 연구자 1명 = 1 Point + nested 배열 | **chunk 1개 = 1 Point** |
| **검색 표현** | 4브랜치 named vector(basic/art/pat/pjt) | **단일 dense+sparse**, doc_type은 payload 필터 |
| **도메인 수** | 4 | **11 doc_type (4 family)** — 평가이력·전문성 신호 신규 |
| **후보 단위** | Point가 곧 연구자 | chunk 검색 후 `researcher_id`로 **집계**하여 후보 생성 |
| **evidence 참조** | `paper:0` (배열 인덱스) | **`chunk_id`** (불변 id) |
...
평가위원 추천 RAG 시스템의 기준 설계를 chunk 데이터 모델 위에서 고정하기 위한 실행 문서다.

- **포함:** 컬렉션/벡터 설계, 검색·집계 전략, hard filter, evidence 선별, LLM 추천 규칙, 제한, 방향성
- **비포함:** 프런트 UI 상세, IaC, 인증 세부 구현, 적재(ingestion) 코드 구현 (→ [`../plans/MIGRATION_PLAN.md`](../plans/MIGRATION_PLAN.md))
...
| 항목 | 최종 결정 | 설계 의미 |
|---|---|---|
| 저장 단위 | chunk 1개 = 1 Point, ID=`chunk_id` | 데이터 도착 단위와 저장 단위 일치, 멱등 적재 |
| 후보 단위 | 검색 후 `researcher_id` 집계 = 연구자 1명 | 저장은 chunk, 추천은 연구자 |
| 검색 표현 | 단일 `dense_e5i` + `sparse_splade` | doc_type은 named vector가 아니라 payload 필터 |
...
| evidence 참조 | `chunk_id` 불변 id | 인덱스 기반 id 폐기 |
...
[LLM Query Planner]
   ...
   ↓
[Retrieval Orchestrator]  (chunk 단위)
   1) 키워드 풀: sparse 키워드 검색 → researcher_id 후보 풀
   2) 하이브리드: 풀 내부에서 doc_type별 dense+sparse prefetch → equal RRF (chunk hit)
   3) 집계: researcher_id로 chunk hit 묶어 RRF 누적 (doc_type별 캡) → 연구자 후보
   4) hard filter: event_year/meta count/exclude org (deterministic, recency=OR)
   5) 결정론적 정렬: score desc → name asc → researcher_id asc
   ↓
[Evidence Selector]  (chunk이 곧 근거)
   - 후보별 매칭 chunk을 family별로 모아 query 관련도 재랭크(cross-encoder→lexical 강등)
...
1. **키워드 풀 단계:** `sparse_splade`로 sparse 키워드 검색 → `researcher_id` 후보 풀을 중복 없이 수집. (lexical 그라운딩 우선 확보)
2. **하이브리드 단계:** 후보 풀을 `researcher_id MatchAny`로 제한한 뒤, doc_type(또는 family)별로 dense+sparse `prefetch`를 조립하고 `FusionQuery(RRF)`로 chunk hit을 만든다.

> v1.x의 "브랜치"는 v2.0에서 "doc_type(또는 family) 경로"로 대체된다. 키워드 풀 단계가 빈 풀을 내면 하이브리드는 건너뛰고 빈 결과 + `keyword_stage_candidate_count=0`을 trace에 남긴다.

### 5.3 연구자 집계 (v2.0 신규 핵심)
chunk hit을 `researcher_id`로 묶어 연구자 후보 1건으로 만든다.
- 각 연구자 점수 = 그 연구자의 chunk hit들에 대한 **RRF 누적**.
- **doc_type별 캡:** 한 doc_type에서 상위 `N`개 chunk까지만 점수에 기여(기본 3). 다작 연구자가 한 영역 chunk 수로 순위를 독식하지 못하게 한다.
...
- `event_year >= 올해-N` (+ `doc_type` 한정) — 최근성. **여러 doc_type recency는 OR(min_should, min_count=1)** 로 결합.
- `researcher_meta.*_count >= 임계값` — 최소 실적.
- `highest_degree` — 학위.
- **제외 기관:** `researcher_meta.affiliated_organization` + 매칭 chunk의 `performing_organization`/`managing_agency`/`appointing_organization`/`evaluation_agency_name`에 대해 적용. Qdrant 필터로 가능한 부분 + 교차 chunk 배제는 Python post-filter.
...
연구자 집계 단계의 family/doc_type 가중은 **앱단에서** 조정 가능하다(이미 v1.x도 `BRANCH_WEIGHTS`로 앱단 가중을 적용했다). v2.0 기본값은 **equal**. intent가 명확할 때만(예: "평가 경험이 풍부한" → 평가이력 family prior↑) 옵트인으로 켠다. 이는 Qdrant weighted RRF가 아니라 **랭크 누적 가중**이며, 켜더라도 hard filter와 결정론적 정렬을 침해하지 않는다. 설정: `NTIS_DOC_TYPE_PRIORS`(기본 미설정=equal).
...
### 7.2 입력 구성
- 후보 카드(머리): `profile`/`researcher_meta` 요약 + 평가이력 요약.
...
| 구분 | 제한 | 영향 | 대응 |
|---|---|---|---|
| 구조 | 다작 연구자가 chunk 수로 집계 점수를 끌어올릴 수 있음 | 순위 왜곡 | doc_type별 캡 + 집계 정규화 |
| 데이터 | `researcher_meta` 비정규화 → 적재 불일치 시 필터 비결정 | 필터 오작동 | 적재 불변식 검증([`DATA_MODEL.md §6`](DATA_MODEL.md)) |
| 데이터 | 기관명/날짜/동일인 식별 품질 민감 | 필터·추천 품질 | 정규화·검증 파이프라인 |
